...
**Última atualização:** 2026-09-12 (Onda 9 — Domínio LIS no backend: contratos de docs)
...
| Rotas `/lis-imports`, `/lis-budgets`, `/sales`, `/attendants`, `/settings/commissions`, `/reports/executive` | api | ⬜ | — | Conforme `API_CONTRACTS.md` §5c/§6b/§10-12. API dividida em LisImport ∥ LisAnalytics ∥ Sales (3 agentes paralelos, DB primeiro) |
| Testes `backend/tests/lis/` + `rls-onda9.spec` + inventário de isolamento | qa | ⬜ | — | Parser (aliases, PDF disfarçado, serial de data, vírgula decimal, linha sem número rejeitada), import idempotente, analytics (requisição duplicada em 2 números, cap 100%, emissão vs pagamento), sales (escopo do atendente, comissão), attendants, RLS com 2 tenants, `LAB_ROUTES` 72 → ~90 |
