...
export default function SummaryColumn({
  ...
  return (
    <div className="flex flex-col gap-4 p-5 h-full">
      <h2 className="text-heading-32 font-semibold">Resumo</h2>

      <div className="flex-1 overflow-y-auto min-h-0">
        {items.length === 0 ? (
          <p className="text-neutral-600 text-sm">Nenhum exame adicionado</p>
        ) : (
          <div data-testid="summary-items" className="space-y-2">
            {items.map((item) => (
              <div
                key={item.examId}
                className="flex justify-between items-center text-sm p-2 rounded hover:bg-neutral-100"
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="text-neutral-600 hover:text-accent-600 text-lg leading-none"
